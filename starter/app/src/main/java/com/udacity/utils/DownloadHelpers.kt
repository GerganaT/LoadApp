...
fun DownloadManager.determineDownloadStatus(id: Long, context: Context) {
    ...
    if (cursor.moveToFirst()) {
        val currentDownloadStatus = cursor.getInt(cursor.getColumnIndex(
            DownloadManager.COLUMN_STATUS
        ))
//TODO Implement this to show notification and details activity with either success or failure content
        downloadStatus = context.resources.getString(
            when (currentDownloadStatus) {
                DownloadManager.STATUS_SUCCESSFUL -> R.string.notification_success
                DownloadManager.STATUS_RUNNING -> R.string.notification_progress
                DownloadManager.STATUS_FAILED -> R.string.notification_failure
